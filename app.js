// ============================================================
// PT TABLET FOLLOW-UP — Redesigned
// Fixed: reg input bug | Simplified: right panel (Today only)
// ============================================================
...
const fmtReg = (raw) => {
    ...
    // First segment: up to 4 digits
    let result = c.slice(0, 4);
    
    // After 4 chars, add slash + up to 3 letters (uppercase)
    if (c.length > 4) {
        result += '/' + c.slice(4, 7).toUpperCase();
    }
    
    // After 7 chars, add slash + up to 4 digits
    if (c.length > 7) {
        result += '/' + c.slice(7, 11);
    }
...
function PTVisitTracker() {
    ...
    const handleSubmit = (e) => {
        ...
        if (!trimmedReg||!vDate||!nvDate) { alert('Please fill all fields'); return; }
        if (!/^\d{4}\/[A-Z]{3}\/\d{4}$/.test(trimmedReg)) { alert('Invalid format\n\nExample: 2026/ABC/0001'); return; }
        if (new Date(nvDate)<=new Date(vDate)) { alert('Next Visit must be after Visit Date'); return; }
        const days = calcDays(vDate, nvDate);
        const prev = getTotalTablets(trimmedReg);
        setVisits([{ id:Date.now(), regNumber:trimmedReg, visitDate:new Date(vDate), nextVisitDate:new Date(nvDate), tabletDays:days, completed:false, recordedAt:new Date() }, ...visits]);
        showToast(`✓ ${days} days · Total: ${prev+days}d`);
        ...
        <div className="autocomplete">
            {items.map(s => { const p=patientData[s.toUpperCase()]; return (
                <div key={s} className="ac-item" onMouseDown={()=>onSelect(p.regNumber)}>
                    ...
                    <span className="ac-meta">💊 {p.totalDays}d · {p.visits.length} visits</span>
                </div>
            );})}
        </div>
    );
            ...
            <>
                ...
                <div className="stats-row">
                    ...
                    <div className="stat-tile">
                        <div className="st-label">Completed</div>
                        <div className="st-value" style={{color:'var(--emerald)'}}>{doneToday}</div>
                ...
                <div className="card">
                    <div style={{marginBottom:'1rem'}}>
                        <div className="card-title" style={{fontSize:'1.1rem', marginBottom:'0.18rem'}}>Today's Schedule</div>
                        <div style={{fontFamily:'var(--font-m)', fontSize:'0.65rem', color:'var(--teal)', letterSpacing:'0.05em'}}>{todayStr.toUpperCase()}</div>
                    </div>
                    ...
                    {todayVisits.length === 0 ? (
                        ...
                        <>
                            <div className="pending-summary">
                                <div className="pending-count">{pendingToday}</div>
                                <div>
                                    <div className="pending-label" style={{fontWeight:600}}>Patients pending</div>
                                    <div className="pending-sub">{doneToday} completed · {todayVisits.length} total today</div>
                                </div>
                            </div>
                            <div className="patient-list" style={{maxHeight:'320px'}}>
                                {todayVisits.map(v=>(
                                    <div key={v.id} className={`prow${v.completed?' done':''}`}>
                                        <span className="preg">{v.regNumber}</span>
                                        <span className="badge bt">💊 {v.tabletDays}d</span>
                                        <span className={`badge ${v.completed?'bg':'ba'}`}>{v.completed?'✓ Done':'Pending'}</span>
                ...
                <div className="card">
                    <div className="card-header">
                        ...
                        <span style={{fontFamily:'var(--font-m)', fontSize:'0.6rem', color:'var(--txt-3)'}}>7 DAYS</span>
                    </div>
    ...
    const AssignPanel = () => (
        <div className="card">
            <div className="card-header">
                ...
                {isExisting && <span className="badge bt" style={{fontSize:'0.6rem'}}>Returning</span>}
            </div>
            <form onSubmit={handleSubmit} autoComplete="off">
                <div className="form-group" style={{position:'relative'}}>
                    ...
                    <input
                        type="text"
                        className="form-input reg-input"
                        value={reg}
                    ...
                    {isExisting && <div className="patient-notice">Returning · {getTotalTablets(reg)}d dispensed so far</div>}
                </div>
                ...
                {preview !== null && preview > 0 && (
                    <div className="tablet-preview">
                        <div>
                            <div className="tp-num">{preview}</div>
                            <div className="tp-label">Days</div>
                        </div>
                        <div style={{flex:1}}>
                            <div className="tp-sub">Tablets to dispense</div>
                            {isExisting && <div className="tp-sub" style={{marginTop:'0.18rem', color:'var(--teal)'}}>New total: {getTotalTablets(reg)+preview}d</div>}
                        </div>
                    </div>
                )}
                        ...
                        <div key={p.regNumber} className="lookup-card" onClick={()=>openHistory(p.regNumber)}>
                            <div className="lc-info">
                                ...
                                <div className="lc-badges">
                                    ...
                                    <span className="badge" style={{background:'rgba(168,85,247,0.12)',color:'#C084FC',border:'1px solid rgba(168,85,247,0.25)'}}>× {p.visits.length}</span>
                                    <span className={`badge ${pending>0?'ba':'bg'}`}>{pending>0?`⏳ ${pending}`:'✓ Done'}</span>
                                </div>
                            </div>
        ...
        <div>
            ...
            <header className="topbar">
                <div className="topbar-brand">
                    ...
                    <div>
                        <div className="topbar-title">PT Follow-up</div>
            ...
            <div className="app-wrap">
                <div className="app-container">
                    {/* LEFT — Assign + mini calendar on desktop */}
                    <div className={`panel-left${activeTab==='assign'?' tab-active':''}`}>
                        ...
                        <div className="desktop-only-card">
                            <div className="card">
                                <div className="card-header">
                                    <span className="card-title">{new Date().toLocaleDateString('en-US',{month:'long'})}</span>
                            ...
                            <div className="card" style={{padding:'0.85rem'}}>
                                <div className="card-title" style={{marginBottom:'0.7rem'}}>Data</div>
                                <div className="data-acts">
                    ...
                    <div className="panel-right">
                        <div style={{display: activeTab==='home'     ? 'contents' : 'none'}}><HomePanel/></div>
                        <div style={{display: activeTab==='calendar' ? 'contents' : 'none'}}><CalendarPanel/></div>
                        <div style={{display: activeTab==='records'  ? 'contents' : 'none'}}><RecordsPanel/></div>
                    </div>
                </div>
            </div>
                ...
                <div className="modal-overlay" onClick={()=>setModalDate(null)}>
                    <div className="modal" onClick={e=>e.stopPropagation()}>
                        ...
                        <div className="modal-head">
                            <div className="modal-title">{new Date(modalDate).toLocaleDateString('en-US',{weekday:'short',month:'long',day:'numeric'})}</div>
                        ...
                        {patientsForDate(modalDate).length===0 ? (
                            ...
                            <>
                                <div style={{background:'var(--teal-dim)',border:'1px solid var(--teal-border)',borderRadius:'var(--radius-sm)',padding:'0.6rem 0.85rem',marginBottom:'0.85rem',fontSize:'0.72rem',color:'var(--txt-2)',fontFamily:'var(--font-m)'}}>
                                    <strong style={{color:'var(--teal)'}}>{new Set(patientsForDate(modalDate).filter(v=>!v.completed).map(v=>v.regNumber.trim().toUpperCase())).size}</strong> pending · {new Set(patientsForDate(modalDate).map(v=>v.regNumber.trim().toUpperCase())).size} total
                                </div>
                                <div className="patient-list" style={{maxHeight:'55vh'}}>
                                    {patientsForDate(modalDate).map(v=>(
                                        <div key={v.id} className={`prow${v.completed?' done':''}`}>
                                            <span className="preg">{v.regNumber}</span>
                                            <span className="badge bt">💊 {v.tabletDays}d</span>
                                            <span className="pmeta" style={{flex:1,textAlign:'right'}}>{v.completed?'✓':'Pending'}</span>
                                            <input type="checkbox" className="checkbox" checked={v.completed} onChange={()=>toggleDone(v.id)}/>
                                        </div>
                                    ))}
                                </div>
            ...
            {showYear && (
                <div className="modal-overlay" onClick={()=>setShowYear(false)}>
                    <div className="modal wide" onClick={e=>e.stopPropagation()}>
                        <div className="modal-handle"/>
                        <div className="modal-head">
                            <div className="modal-title">Year — {curYear}</div>
                ...
                <div className="modal-overlay" onClick={()=>setShowHistory(false)}>
                    <div className="modal" onClick={e=>e.stopPropagation()}>
                        ...
                        <div className="hist-head">
                            <div>
                                <div className="hl">Total Tablets</div>
                                <div className="hv">{selPatient.totalDays}</div>
                                <div className="hs">days dispensed</div>
                            </div>
                            <div>
                                <div className="hl">Visits</div>
                                <div className="hv">{selPatient.visits.length}</div>
                            ...
                            {[...selPatient.visits].reverse().map(v=>(
                                <div key={v.id} className="visit-card">
                                    ...
                                    <div style={{textAlign:'right',flexShrink:0}}>
                                        <div className="vc-days">{v.tabletDays}</div>
                                        <div className="vc-dlbl">days</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                        <div style={{marginTop:'0.9rem',padding:'0.9rem',background:'var(--teal-dim)',border:'1px solid var(--teal-border)',borderRadius:'var(--radius-sm)',textAlign:'center'}}>
                            <div style={{fontSize:'0.58rem',color:'var(--teal)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:'0.25rem',fontFamily:'var(--font-m)'}}>Treatment Duration</div>
                            <div style={{fontSize:'0.9rem',fontWeight:600,color:'var(--txt-1)',fontFamily:'var(--font-m)'}}>{fmtShort(selPatient.firstDate)} → {fmtShort(selPatient.lastNext)}</div>
                            <div style={{fontSize:'0.68rem',color:'var(--txt-3)',marginTop:'0.2rem',fontFamily:'var(--font-m)'}}>{Math.ceil((new Date(selPatient.lastNext)-new Date(selPatient.firstDate))/86400000)} days</div>
                        </div>
                    </div>
                </div>
            )}
